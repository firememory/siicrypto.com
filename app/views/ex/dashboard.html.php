...
foreach($trades as $tr){
	...
	foreach($VirtualCurr as $currency){ ?>
		<td style="text-align:center"><a href="/users/funding/<?=$currency?>" class="btn btn-primary btn-sm btn-block"> <?=$currency?> </a><br><?=substr(number_format($details['balance'][$currency]/100000000,8),0,-6)?><small><?=substr(number_format($details['balance'][$currency]/100000000,8),-6)?></small></td>
	<?php }
	if($all){
		foreach($FiatCurr as $currency){ ?>
			<td style="text-align:center"><a href="/users/funding_fiat/<?=$currency?>" class="btn btn-primary btn-sm btn-block"> <?=$currency?> </a><br><?=substr(number_format($details['balance'][$currency]/100,4),0,-2)?></td>
		<?php }
	} //if all
?>
</tr></table>
